...
async def emoji_cloner():
    clear()
    logo()
    guild_id = input(f"                {b}[{w}Kopyalamak İstediğin Sunucu : {b}]{w} >> ")
    clear()
    logo()
        ...
        if guild_info_response.status_code in [200, 204]:
            ...
            for emoji in data:
                ...
                with open(file_path, "wb") as f:
                    ...
                    f.write(emoji_data)
                t = current_time()
                print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} İndirildi: {emoji_name} ({emoji_id})")
        else:
            t = current_time()
            print(f"                {b}[{w}{t}{b}]{w} {r}[-]{w} Klasör Oluşturulamadı")
    else:
        t = current_time()
        print(f"                {b}[{w}{t}{b}]{w} {r}[-]{w} Klasör Oluşturuldu")